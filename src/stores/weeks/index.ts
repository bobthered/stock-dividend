	...
	($settings) =>
		[...Array($settings.years * 52)].reduce(
			({ data, weeks }, _, weekIndex) => {
				...
				data.reinvestment =
					$settings.weeklyReinvestment *
					Math.pow(1 + $settings.annualIncreasePercent, data.year - 1);
				data.totalInvested =
					(data?.totalInvested || $settings.initialInvestment) + data.reinvestment;
				data.portfolio = (data?.portfolio || data.totalInvested) + data.reinvestment;
				data.shares = data.portfolio / $settings.strikePrice;
				...
				data.dividendPayout = data.isLastWeekOfMonth
					? data.shares * $settings.dividendRate
